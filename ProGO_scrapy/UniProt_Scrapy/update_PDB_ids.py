...
UPIDs = pd.read_excel("target_ID.xlsx").sort_values(by="No.").set_index("No.").Human_UPID.values # [:100]
...
for UPID in tqdm.tqdm(UPIDs):
    ...
    # scraping_result.append({ "UPID": UPID, "relatedPDB": pdb_ids })
    query = { "UPID": UPID }
    newvalues = { "$set": { "relatedPDB": pdb_ids } }
...
# import json
# # json.dumps(scraping_result)
# with open('scraping_result.json', 'w') as fout:
#     json.dumps(scraping_result, fout)
