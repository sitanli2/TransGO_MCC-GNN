...
for id in all_id:
    if(id.entity_type.string == 'COMPOUND' and id.status.string == 'ACTIVE'):
        ...
        if(len(unactivate_id_list) < 5):#控制爬取不少于5个非激活化合物（非活性物ID？？？）
            unactivate_id_list.append(id.chembl_id.get_text())
    if(id.entity_type.string == 'ASSAY' and id.status.string == 'ACTIVE'):
    ...
    #     break
    if(id.entity_type.string != 'COMPOUND' and id.entity_type.string != 'ASSAY'):#想找到除chembl id 和assay id之外的id
        print('此类即不属于compound又不属于assay的id的类型为：',id.entity_type.string)

print('chembl id个数:',len(chembl_id_list),chembl_id_list)
print('assay id个数:',len(assay_id_list),assay_id_list)
print('unactivate_id个数:',len(unactivate_id_list),unactivate_id_list)
...
for i in target_id_list:
    ...
    target_dict['Compounds'] = []#这个需要关联Compound_molecule小分子化合物表 之后再说
    target_dict['Activities'] = []#这个需要关联Activities活性物表 之后再说
