...
'''
devhead = dtmr_dev.get_collection('chembl_Activity')#活性物信息表，作为所有表的主表，里面存了所有的关联id，详情见上方注释

dev = dtmr_dev.get_collection('chembl_Compound_molecule')
dev1 = dtmr_dev.get_collection('chembl_Assay')
dev2 = dtmr_dev.get_collection('chembl_Target')
dev3 = dtmr_dev.get_collection('chembl_Document')
dev4 = dtmr_dev.get_collection('chembl_Drugs')

devlog = dtmr_dev.get_collection('chembl_Log')#用来保存当日的log日志下次的断点就设在前一天的start_point offset + limit


limit=input("你每轮循环需要几条activities id？？(推荐条数50条)")
start_point=input('新的断点从第几条数据开始？？')
activity_no = input('你此次运行想要爬到哪里为止？？')

while(int(start_point) <= int(activity_no)):#循环结束将抓取前 5000 条的activities内容
    #print('ok')
    '''activities 活性物接口爬取'''
    ...
    activities_id_list = []
    molecule_id_list = []#去重过后的id表，避免重复存取
    target_id_list = []
    assay_id_list = []
    document_id_list = []

    # relate_list_molecule = []#完整的没去重的id表，方便后续找一对多，和多对多交叉关联
    # relate_list_target = []
    # relate_list_assay = []
    ...
    while(i <= len(all_activities)-1 ):
        ...
        print("此时的activity id=",all_activities[i].activity_id.string)
        ...
        if (all_activities[i].activity_id.string not in activities_id_list):#其实activity_id 是主表的主键，他是唯一的，不需要这个条件判断。但为了防止万一还是加上了。
            if(devhead.count_documents({"activity_id":all_activities[i].activity_id.string}) == 0):
                activities_id_list.append(all_activities[i].activity_id.string)


        if(all_activities[i].molecule_chembl_id.string not in molecule_id_list):
        ...
        activitise_dict['value'] = all_activities[i].value.string
        activitise_dict['upper_value'] = all_activities[i].upper_value.string

        if(bool(all_activities[i].pchembl_value)):#当相关属性可能不存在时，需要先条件判断
    ...
    【注意】还需和每个化合物小分子对应的target进行关联得到targets信息，再插入molecule_dict中，说明此化合物有活性的对应的靶点，或该靶点对应的活性化合物小分子。
    '''
    ...
    for i in molecule_id_list:
        ...
        molecule_dict['chebi_par_id'] = soup.find('chebi_par_id').get_text()#get_text()和.string作用一致，都是返回对应解析路径下的内容，但当路径下为空时后者会直接报错。
        ...
        molecule_inner_properties['num_lipinski_ro5_violations'] = soup.find('molecule_properties').num_lipinski_ro5_violations.string
    ...
    for element in document_list:#避免重复存取，只存储没有的，或更新现有的
    ...
    start_point = int(start_point) + int(limit) #下一轮内循环的start point

devlog.insert_one(log)#这个log的插入一般放在循环的最后再插入，因为如果运行中途发生了错误如断网，或者某些不确定因素运行中断时，log将无法插入，这也是一个判断程序是否完整执行的有效手段。

'''
1. chembl_refine.py 介绍：
作为chembl4.0.py的精炼版本，该版本删除了一些没必要的属性和方法，优化了代码运行效率，优化了判重，更新等操作的时间复杂度，但缺点是删减了很多表项内容，以及功能。

2. chembl_refine.py 使用手册：
2.1 运行该程序后会让使用者输入三个参数：
...
2.1.3 你此次运行想要爬到哪里为止？？
这个参数是用于说明你此次运行程序在遍历到第几条activity_id的时候终止（总数据量太大，如果不让他在中途终止的话若发生断电，断网等情况时会有巨大损失）
...
2.5 如果程序运行过程中发生断电、断网等不确定因素，即整个循环并没有结束就被强行打断，则代码不会生成最后的log，同理没有log说明运行过程中发生错误，但这并不说明断网之前的那些数据也没了。
