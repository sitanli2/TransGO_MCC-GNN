...
sql_indication = "select record_id from drug_indication limit 0,20"
...
for record in relist_no_repeat_indication:
    ...
    while(i <= len(relist2)-1):
        indication = {}
        indication["drugind_id"] = relist2[i][0]
        indication["record_id"] = relist2[i][1]
        indication["molregno"] = relist2[i][2]
        indication["max_phase_for_ind"] = str(relist2[i][3]) #decimal 类型需要转字符串
        indication["mesh_id"] = relist2[i][4]
        indication["mesh_heading"] = relist2[i][5]
        indication["efo_id"] = relist2[i][6]
        indication["efo_term"] = relist2[i][7]
        tempdict = copy.deepcopy(indication)
        drug_indication.append(tempdict)
        i += 1
    #record_dict[str(record_id)] = drug_indication
    #record_dict[record_id] = drug_indication
# print("debug=",record_dict["1343055"])
templist = copy.deepcopy(drug_indication)
# for record in relist_no_repeat_indication:
#     record_id = record[0]
#     for diction in templist:
#         if(diction["record_id"] == record_id):
#             record_dict[str(record_id)] = diction
drug_indication_summary.append(tempdict)


print("indication summary = ",drug_indication_summary[0])
print("indication length = ",len(drug_indication_summary))
print("===========================================")
...
sql_mechanism = "select record_id from drug_mechanism limit 0,20"
...
for record in relist_no_repeat_mechanism:
    ...
    while (i <= len(relist2) - 1):
        ...
        tempdict = copy.deepcopy(mechanism)
        drug_mechanism.append(tempdict)
        i += 1

    record_dict[str(record_id)] = drug_mechanism
    tempdict = copy.deepcopy(record_dict)
    drug_mechanism_summary.append(tempdict)

print("mechanism summary = ", drug_mechanism_summary[0])
print("===========================================")
...
sql_warning = "select record_id from drug_warning limit 0,20"
...
for record in relist_no_repeat_warning:
    ...
    while (i <= len(relist2) - 1):
        ...
        tempdict = copy.deepcopy(warning)
        drug_warning.append(tempdict)
        i += 1
...
遍历 metabolism (新陈代谢)
'''
sql_metabolism = "select drug_record_id from metabolism limit 0,20"
...
'''
returnIDdict(ID,list) 函数是为了通过record_id在四张列表里面找他们是否存在，存在就返回以ID为keyword的值，不存在就返回0
'''
def returnIDdict(ID,list):
    # for ite in list:
