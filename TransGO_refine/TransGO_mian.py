import argparse, os
from utils import loading_contact_map, loading_seq, get_label, get_label_test, loading_item_data, PFPDataset, collate,ESMDataset,ESM_bilstm_Dataset #大批量测试数据 Database1 or Database1+
...
from model import DeepFRI,Model_Net_ESM_biLSTM_upgrade,Model_Net_ESM_biLSTM_CombineUpgrade,TCC_GNN
...
def train(args):
    ...
    for data_source in args.data:
        if data_source == 'Database1+' or data_source == 'Dataset_Ultimate':
            ...
            dict_contact_pdb = loading_contact_map(data_source, args.threshold,MapDensification=args.MapDensification)
            dict_contact_pdb2 = loading_contact_map(data_source, args.threshold, MapDensification=args.MapDensification) #第二类edge_index
            ...
            dict_contact_pdb = loading_contact_map(data_source, args.threshold,MapDensification = "False")
            dict_contact_pdb2 = loading_contact_map(data_source, args.threshold,MapDensification = "False")  # 第二类edge_index,MapDensification = "True"
            print('loading a small batch of seq from Database1 i.e uniprot sequence...')
            dict_seq_pdb = loading_seq(data_source)

    for func in args.func:
        label, label_num = get_label(func) #如果是小批量测试，就用这个get_label_test, 否则就要用 get_label
        train_item = loading_item_data(func, 'train',data_source='other_wise') #小批量测试加上 data_source='testData',其他时候data_source='other_wise'
        valid_item = loading_item_data(func, 'valid',data_source='other_wise')
        # test_item = loading_item_data(func, 'test',data_source='other_wise')
        print('############################################################')
        ...
        # dataset_train = DataLoader(train_data, batch_size=5, shuffle=True, collate_fn=collate, drop_last=False)
        # dataset_valid = DataLoader(valid_data, batch_size=5, shuffle=False, collate_fn=collate, drop_last=False)
        """
        ...
        if (args.model == 'TransGO' and args.seq_feature == 'Esm+bilstm'):
            ...
            """
            model = Model_Net_ESM_biLSTM_CombineUpgrade(output_dim=label_num, net=args.net, hidden_dim=args.hidden_dim, pool=args.pool,dropout=args.dropout).to(device) #ESM+bilstm特征
        elif (args.model == 'TransGO' and args.seq_feature == 'bilstm'):
            #model = Model_Net_ESM_biLSTM_CombineUpgrade3(output_dim=label_num, net=args.net, hidden_dim=args.hidden_dim,pool=args.pool, dropout=args.dropout).to(device)
            model = TCC_GNN(output_dim=label_num, net=args.net, hidden_dim=args.hidden_dim,pool=args.pool, dropout=args.dropout).to(device)
        ...
        for e in range(args.Epoch):
            ...
            for batch_idx, batch in enumerate(dataset_train):  # 遍历了(enumerate 枚举) 训练数据集中的每个批次（batch_size=64）数据
                ...
                if (args.net != 'combine_net'):
                    ...
                    """根据索引重新制作一个全新的GCN DATA 来替换先前的edge_index"""
                    data_prot = batch.to(device)
                    uidlist = data_prot.uid  # 拿到这一个batch的所有UPID索引列表
                    dict_contact_pdb2_thisbatch = {key: dict_contact_pdb2[key] for key in uidlist if
                                                   key in dict_contact_pdb2}  # 生成这一个batch的子字典
                    label_thisbatch = {key: label[key] for key in uidlist if key in label}
                    dict_seq_pdb_thisbatch = {key: dict_seq_pdb[key] for key in uidlist if key in dict_seq_pdb}
                    # for UPID in uidlist:
                    #     print(UPID)
                    train_data_thisbatch = PFPDataset(train_data_X=uidlist,
                                                      train_contactmap_X=dict_contact_pdb2_thisbatch,
                                                      train_feature_matrix_X=dict_seq_pdb_thisbatch,
                                                      train_data_Y=label_thisbatch,
                                                      model_feature=model_feature, device=device)
                    dataset_train_thisbatch = DataLoader(train_data_thisbatch, batch_size=len(uidlist), shuffle=True,
                                                         collate_fn=collate, drop_last=False)
                    batch_idx_thisbatch = 0  # 由于只有一个 batch，batch_idx 可以固定为 0
                    batch_thisbatch = next(iter(dataset_train_thisbatch))
                    # 查看 batch 内容
                    print(batch_idx_thisbatch, batch_thisbatch)
                    data_prot_replace = batch_thisbatch.to(device)

                    optimizer.zero_grad()
                    output = model(data_prot, data_prot_replace)  # 传入需要替换的接触图，在训练TransGO时在forward函数中进行接触图转换
                    loss = bceloss(output, data_prot.label)  # 二元交叉熵损失函数
                    loss.backward()  # 误差反向传播，更新权重参数
                    optimizer.step()  # Adam优化器
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(dataset_valid):  # 遍历了(enumerate 枚举) 验证数据集中的每个批次数据
                    if (args.net != 'combine_net'):
                        data_prot = batch.to(device)
                        #output_valid = model(data_prot,data_prot_replace)
                        ...
                        train_data_thisbatch = PFPDataset(train_data_X=uidlist,
                                                          ...
                                                          train_data_Y=label_thisbatch,
                                                          model_feature=model_feature, device=device)
                        dataset_train_thisbatch = DataLoader(train_data_thisbatch, batch_size=len(uidlist),
                                                             shuffle=True,
                                                             collate_fn=collate, drop_last=False)
                        batch_thisbatch = next(iter(dataset_train_thisbatch))
                        # batch_idx_thisbatch = 0  # 由于只有一个 batch，batch_idx 可以固定为 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=lambda s: [item for item in s.split(",")], default=['Database1+'], help="choose your experiment dataset")  # , 'alpha','Database1+'，Dataset_Ultimate,testData
    ...
    parser.add_argument('--model', type=str, default='TransGO', help="protein function prediction model(DeepFRI,TransGO,TransGO+.....)")
