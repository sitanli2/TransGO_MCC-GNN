...
def embed_sequence(model, x, pool='none', use_cuda=False,device = None): #传进来的这个model就是model_feature,最后返回提取得到的特征矩阵
...
class ProSEMT(nn.Module):
    ...
    def load_pretrained(path='prose_mt'):
        ...
        if path is None or path == 'prose_mt':
            ...
            path = os.path.join(root, 'G:/fkfk/AlphaFold_refine/saved_models', 'prose_mt_3x1024.sav')
        ...
        proj = nn.Linear(proj_in, 100) #经过debug后发现，proj这个全连接层输入即proj_in = encoder.proj.in_features = 6165，输出正如该行所示被设定为100
        encoder.proj = proj #更新encoder即实例化SkipLSTM的proj即全连接层
        encoder.nout = 100

        scop_predict = OrdinalRegression(100, 5, compare=L1(), allow_insertions=False)#正如上面所示输出100再进行OrdinalRegression
