import argparse,os
from utils import loading_contact_map,loading_seq,get_label,loading_item_data,PFPDataset,collate,ESMDataset
from torch.utils.data import DataLoader
from model import DeepFRI_ESM_33,DeepFRI_ESM_6
    ...
    for func in args.func:
        ...
        for data_source in args.data:
            ...
            if data_source == 'Database1+':
                ...
                elif args.seq_feature == "ESM":
                    train_data = ESMDataset(train_data_X=train_item, train_contactmap_X=dict_contact_pdb,train_feature_matrix_X=dict_seq_pdb, train_data_Y=label, device=device)
                    valid_data = ESMDataset(train_data_X=valid_item, train_contactmap_X=dict_contact_pdb,train_feature_matrix_X=dict_seq_pdb, train_data_Y=label, device=device)
            ...
            model = DeepFRI_ESM_6(output_dim=label_num,net=args.net,hidden_dim=args.hidden_dim,pool=args.pool,dropout=args.dropout).to(device)
            ...
            for e in range(args.Epoch):
                ...
                if perf['all']['m-aupr']>m_AUPR:
                    ...
                    torch.save(model.state_dict(),'../UltraDataset/Dataset_Ultimate/' + str(func) + '/model/' + str(data_source) + '_' + str(func) + '_' + args.net + '_' + str(args.hidden_dim) + '_' + args.pool + '_' + str(args.dropout) + '_' + str(args.threshold)+'_'+str(args.seq_feature) + '.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', type=lambda s: [item for item in s.split(",")],
                        default=['Database1+'], help="data source.")#, 'alpha'
    ...
    parser.add_argument('--net', type=str, default='GCN', help="GCN or GAT,GATv2 for model")
    ...
    parser.add_argument('--seq_feature', type=str, default='ESM',help="The method(ESM6,ESM33,bilstm,Esm+bilstm.....) to extract amino acid feature from sequence")
