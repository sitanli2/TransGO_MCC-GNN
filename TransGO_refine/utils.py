...
"""使用大批量数据（如Database1+时）测试时使用该工具包"""

def loading_contact_map(data_source,threshold,MapDensification="False"): #data_source = Database1+ or Dataset_Ultimate 即扩充后的混和数据集
    contact_map_list = os.listdir('../UltraDataset/'+data_source+'/contact_map_dense_pdb_'+str(threshold)) #../UltraDataset/Database1_Uniprot+PDB/ or ../UltraDataset/Database1+/
    dict_contact_map = {}
    for item in tqdm(contact_map_list):
        if data_source == 'Database1+' or data_source == 'Dataset_Ultimate':
            if MapDensification == "True": #使用扩充隐藏边后的接触图
                pid_chain_uid = item.strip().split('.')[0]
                dict_contact_map[pid_chain_uid] = np.load('../UltraDataset/' + data_source + '/contact_map_dense_densification_' + str(threshold) + '/' + item)
            else:
                pid_chain_uid = item.strip().split('.')[0]
                dict_contact_map[pid_chain_uid] = np.load('../UltraDataset/'+data_source+'/contact_map_dense_pdb_'+str(threshold)+'/'+item)
            ...
            pid_chain_uid = item.strip().split('.')[0]
            dict_contact_map[pid_chain_uid] = np.load('G:/fkfk/AlphaFold_refine/UltraDataset/testData/contact_map_dense_pdb_' + str(threshold) + '/' + item)
...
def loading_seq(data_source):
    dict_seq = {}
    if data_source == 'Database1+' or data_source == 'Dataset_Ultimate':
        with open('../UltraDataset/'+data_source+'/pdb_mapping_seq.txt','r') as f:
        ...
        with open('../biLSTM+GCN/data_collect/alpha_mapping_seq.txt') as f:
            for line in tqdm(f,total=7485):
                if '>' in line:
                    pid_chain_uid = line[1:].strip('\n').split('\t')[0]
                    seq_pdb = line[1:].strip('\n').split('\t')[1]
                    dict_seq[pid_chain_uid] = seq_pdb
        ...
        with open('G:/fkfk/AlphaFold_refine/UltraDataset/testData/pdb_mapping_seq.txt','r') as f:
            for line in tqdm(f,total=10): #精炼后的数据只有3000条了
                if '>' in line:
                    pid_chain_uid = line[1:].strip('\n').split('\t')[0]
                    seq_pdb = line[1:].strip('\n').split('\t')[1]
                    dict_seq[pid_chain_uid] = seq_pdb
...
def get_label_test(func='mf'): #默认只测试 mf 标签
    if func == 'mf':
        mf_dict = {}
        mf_func = []
        mf_label_dict = {}
...
def loading_item_data(func,name,data_source='notest'):
    if (data_source == 'testData'):
        item = []
        with open('../UltraDataset/testData/' + func + '/' + name + '_data.txt', 'r') as f:
            for line in f:
                line = line.strip('\n')
                item.append(line)
        return item
        ...
        item = []
        with open('../UltraDataset/Database1+/'+func+'/'+name+'_data.txt','r') as f:
            for line in f:
                line = line.strip('\n')
                item.append(line)
        return item
...
class ESMDataset(InMemoryDataset): # 定义子类ESMDataset继承InMemoryDataset
    ...
    def __getitem__(self, idx):
        ...
        if (model_location == "esm1b_t33_650M_UR50S"): # G:/Edge_DownLoad/esm1b_t33_650M_UR50S
            with torch.no_grad():
                results = model(batch_tokens, repr_layers=[33], return_contacts=True)
            token_representations = results["representations"][33]
        else:  # 只有6层的esm2_t6_8M_UR50D
...
class ESM_bilstm_Dataset(InMemoryDataset): # 定义子类ESMDataset继承InMemoryDataset
    ...
    def __getitem__(self, idx):
        ...
        GCNData_mol = DATA.Data(x=combined_feature_matrix,
                                edge_index=torch.LongTensor(contact_map),
                                label=torch.FloatTensor([label]),uid = self.X_data_list[idx])
